...
<div id="wrapper">
    ...
    <h1>Register for My WiFind</h1>
        <p>First Name:</p><input type="text" pattern="[A-Za-z]*" title = "Please only use a-z and A-Z" id="f_name" name="f_name" required placeholder="First Name" <?php if ($error) echo "value ='".$_POST['f_name']."'";?>><br>
       
	   <p>Last Name:</p><input type="text" pattern="[A-Za-z]*" title = "Please only use a-z and A-Z" name="l_name" required placeholder="Last Name"<?php if ($error) echo "value ='".$_POST['l_name']."'";?>><br>
       
	   <p>Age:</p><input type="number" min="1" max="99" name="age" required placeholder="Age" <?php if ($error) echo "value ='".$_POST['age']."'";?>><br>
      
	  <p>Gender:</p><select id='gender' required name="gender">
            <option <?php if ($error && $_POST['gender'] == "m") echo "selected='selected'"?>value="m">Male</option>
            <option <?php if ($error && $_POST['gender'] == "f") echo "selected='selected'"?>value="f">Female</option>
            <option <?php if ($error && $_POST['gender'] == "o") echo "selected='selected'"?>value="o">Prefer not to say</option>
        </select><br>
     
	   <p>How excited for free wifi are you?</p>
	   <input type="range" size="2" required name="excitement" min="1" max="10" value="<?php if ($error) {echo $_POST['excitement'];}else{echo "5";}?>"><br>
       
       
	   <p>Profile Color:</p><input type="color" name="profile_color" pattern = "[A-Fa-f0-9]{6}*" required value="<?php if ($error) {echo $_POST['profile_color'];}else{echo "#00aa00";}?>"><br>
        
	   <p>Email Address:</p><input type="email" name="email" required placeholder="someone@example.com"<?php if ($error) echo "value ='".$_POST['email']."'";?>><br>
